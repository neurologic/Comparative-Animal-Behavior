...
Answer all questions either electronically or by hand. Label each of your answers with the corresponding question number (Q1 - Q). Scan or convert your document to a good quality PDF. Upload to gradescope by the deadline. 
...
**Figure 4**: Correlations between behavioral traits in F2 offspring. The correlation (R2) between two traits is written in each square and color coded by value. The only significant correlation was between the two escape traits: Angular Speed and Peak Angle (correlation = 0.28).[^oliva-2022]
...
d) These behavioral phenotypes are genetically inherited.  
